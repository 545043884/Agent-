...
def load_system_prompts():
    try:
        ...
        logger.error("[load_system_prompts]在yaml配置文件中没有main_prompt_path得配置项")
        raise e
    try:
        return  open(system_prompts_path,"r",encoding="utf-8").read()
    except Exception as e:
        logger.error("[load_system_prompts]在yaml配置文件中没有main_prompt-path得配置项")
        raise e
...
def load_rag_prompts():
    try:
        ...
        logger.error(f"[load_rag_prompts]在yaml配置文件中没有rag_summarize_prompt_path得配置项，{str(e)}")
        raise e
    try:
...
def load_report_prompts():
    try:
        ...
        logger.error("[load_system_prompts]在yaml配置文件中没有main_report-path得配置项")
        raise e
    try:
        return  open(system_report_path,"r",encoding="utf-8").read()
    except Exception as e:
        logger.error(f"[load_report_prompts]在yaml配置文件中没有main_report-path得配置项，{str(e)}")
        raise e
